...
void initItems(void) {
    ...
    while (itemsInserted != numItems) {
        ...
        if (items[randIndex].isEligible() && items[randIndex].getRarity() < randRarity) {
            if (items[randIndex].isArtifact() && !contains(indexesOfItemArtifactsInserted, randIndex)) {
            ...
            else if (!items[randIndex].isArtifact()) { // non artifact items
                dungeon.getMap()[items[randIndex].getPosY()][items[randIndex].getPosX()] = cell_t { symbol, -2 };
                itemsInserted++;
                dungeonItems.push_back(items[randIndex]);
            }
        }
...
void printCharacter(int x, int y) {
    short color = 0;
    for (Item i: dungeon.getDungeonItems()) {
        if (i.getPosY() == y && i.getPosX() == x && dungeon.getMap()[y][x].hardness == -2) {
            color = i.getColor()[0];
            break;
        }
    }
